...
import { FaUserAlt, FaEnvelope, FaSuitcase, FaMoneyBillAlt, FaBullseye } from 'react-icons/fa';
            ...
            <div className="card bg-white shadow-md w-full max-w-md mt-6 p-6">
                ...
                <div className="space-y-4 text-gray-700 text-lg">
                    <div className="flex items-center">
                        ...
                        <p><span className="font-semibold text-gray-900">User Name:</span> {name}</p>
                    </div>
                    <div className="flex items-center">
                        <FaEnvelope className="text-gray-500 mr-3" />
                        <p><span className="font-semibold text-gray-900">Email:</span> {email}</p>
                    </div>
                    <div className="flex items-center">
                        <FaSuitcase className="text-gray-500 mr-3" />
                        <p><span className="font-semibold text-gray-900">Travel Type:</span> {travel_group}</p>
                    </div>
                    <div className="flex items-center">
                        <FaMoneyBillAlt className="text-gray-500 mr-3" />
                        <p><span className="font-semibold text-gray-900">Budget:</span> ${min_budget} - ${max_budget}</p>
                    </div>
                    <div className="flex items-center">
                        <FaBullseye className="text-gray-500 mr-3" />
                        <p><span className="font-semibold text-gray-900">Preference:</span> {cleanedPurpose}</p>
                    </div>
                </div>
            </div>
            ...
            {itineraries.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    {itineraries.map((itinerary: any) => (
                        <div key={itinerary.id} className="card bg-base-100 shadow-lg hover:shadow-xl transition-all">
                            <figure>
                                <img
                            ...
                            <div className="card-body p-4">
                                <h2 className="card-title text-xl font-bold text-gray-900">{itinerary.destination}</h2>
                ...
                <p className="text-lg text-center text-gray-600 mt-6">No itineraries available.</p>
            )}
        </div>
    );
